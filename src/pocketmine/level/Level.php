...
//		}
	...
	public function addEntityMotion($viewers, $entityId, $x, $y, $z) {
		...
		foreach($viewers as $p) {
			if(!isset($this->motionToSend[$p->getIdentifier()])) {
				$this->motionToSend[$p->getIdentifier()] = ['data' => []];
			}
	...
	public function addEntityMovement($viewers, $entityId, $x, $y, $z, $yaw, $pitch, $headYaw = null) {
		...
		foreach($viewers as $p) {
			if(!isset($this->moveToSend[$p->getIdentifier()])) {
				$this->moveToSend[$p->getIdentifier()] = ['data' => []];
			}
	...
	public function updateChunk($x, $z) {
		...
		$data = $chunk->getBlockIdArray() . $chunk->getBlockDataArray() . $chunk->getBlockSkyLightArray() . $chunk->getBlockLightArray() . pack("C*", ...$chunk->getHeightMapArray()) . pack("N*", ...$chunk->getBiomeColorArray()) . $extraData->getBuffer() . $tiles;
