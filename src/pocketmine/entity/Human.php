	...
	public function saveNBT() {
		...
		if($this->inventory !== null) {
			for($slot = 0; $slot < 9; ++$slot) {
				...
				if($hotbarSlot !== -1) {
					...
					if($item->getId() !== 0 and $item->getCount() > 0) {
						$this->namedtag->Inventory[$slot] = new Compound(false, [new ByteTag("Count", $item->getCount()), new ShortTag("Damage", $item->getDamage()), new ByteTag("Slot", $slot), new ByteTag("TrueSlot", $hotbarSlot), new ShortTag("id", $item->getId()),]);
						continue;
					}
				}
				$this->namedtag->Inventory[$slot] = new Compound(false, [new ByteTag("Count", 0), new ShortTag("Damage", 0), new ByteTag("Slot", $slot), new ByteTag("TrueSlot", -1), new ShortTag("id", 0),]);
			}
			...
			for($slot = 9; $slot < $slotCount; ++$slot) {
				...
				$this->namedtag->Inventory[$slot] = new Compound(false, [new ByteTag("Count", $item->getCount()), new ShortTag("Damage", $item->getDamage()), new ByteTag("Slot", $slot), new ShortTag("id", $item->getId()),]);
			}
