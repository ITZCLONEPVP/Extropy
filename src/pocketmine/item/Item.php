...
class Item {
	...
	public function __construct($id, $meta = 0, $count = 1, $name = "Unknown", $obtainTime = null) {
		...
		$this->name = $name;
		if($obtainTime == null) $obtainTime = time();
	...
	private static function initSpecialItems() {
		self::$foods = [Item::APPLE => 4, Item::MUSHROOM_STEW => 6, Item::BEETROOT_SOUP => 5, Item::BREAD => 5, Item::RAW_PORKCHOP => 2, Item::COOKED_PORKCHOP => 8, Item::RAW_BEEF => 3, Item::STEAK => 8, Item::COOKED_CHICKEN => 6, Item::RAW_CHICKEN => 2, Item::MELON_SLICE => 2, Item::GOLDEN_APPLE => 4, Item::PUMPKIN_PIE => 8, Item::CARROT => 3, Item::POTATO => 1, Item::BAKED_POTATO => 5, Item::COOKIE => 2, Item::COOKED_FISH => [0 => 5, 1 => 6], Item::RAW_FISH => [0 => 2, 1 => 2, 2 => 1, 3 => 1],];

		self::$effectItems = [Item::GOLDEN_APPLE => [0 => [Effect::getEffect(Effect::REGENERATION)->setDuration(100)->setAmplifier(1), Effect::getEffect(Effect::ABSORPTION)->setDuration(2400)], 1 => [Effect::getEffect(Effect::REGENERATION)->setDuration(600)->setAmplifier(4), Effect::getEffect(Effect::ABSORPTION)->setDuration(2400), Effect::getEffect(Effect::DAMAGE_RESISTANCE)->setDuration(6000), Effect::getEffect(Effect::FIRE_RESISTANCE)->setDuration(6000)]], Item::RAW_FISH => [3 => [//Effect::getEffect(Effect::HUNGER)->setAmplifier(2)->setDuration(15 * 20),
			Effect::getEffect(Effect::NAUSEA)->setAmplifier(1)->setDuration(15 * 20), Effect::getEffect(Effect::POISON)->setAmplifier(3)->setDuration(60 * 20)]]];
	}
	...
	public function addEnchantment(Enchantment $ench) {
		...
		foreach($tag->ench as $k => $entry) {
			if($entry["id"] === $ench->getId()) {
				$tag->ench->{$k} = new Compound("", ["id" => new ShortTag("id", $ench->getId()), "lvl" => new ShortTag("lvl", $ench->getLevel())]);
				$found = true;
				break;
			}
		}
		...
		if(!$found) {
			$tag->ench->{count($tag->ench) + 1} = new Compound("", ["id" => new ShortTag("id", $ench->getId()), "lvl" => new ShortTag("lvl", $ench->getLevel())]);
		}
	...
	public function setCustomName($name) {
		...
		if(!$hadCompound) $this->setCompound($tag);
	...
	public function isFood() {
		return isset(Item::$foods[$this->getId()]);
	}

	public function getFoodValue() {
		return Item::$foods[$this->getId()];
	}

	public function hasEffects() {
		return isset(Item::$effectItems[$this->getId()]) and isset(Item::$effectItems[$this->getId()][$this->getDamage()]);
	}

	public function getEffects() {
		$effects = Item::$effectItems[$this->getId()][$this->getDamage()];
		if(is_array($effects)) {
			$e = [];
			...
			return $e;
		} elseif($effects instanceof Effect) {
