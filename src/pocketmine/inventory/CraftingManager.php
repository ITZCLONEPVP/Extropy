...
class CraftingManager {
	...
	public function __construct() {
		...
		$this->registerRecipe((new BigShapedRecipe(Item::get(Item::SIGN, 0, 3), "PPP", "PPP", " S"))->setIngredient("S", Item::get(Item::STICK, 0, 1))->setIngredient("P", Item::get(Item::WOODEN_PLANKS, null, 6))); //TODO: check if it gives one sign or three
	...
	protected function registerStonecutter() {
		...
		$buildRecipes[] = ((new ShapedRecipe(Item::get(Item::STONE, Stone::GRANITE, 1), ...$shapes["blockrecipe1X2"]))->setIngredient("A", Item::get(Item::STONE, Stone::DIORITE, 1))->setIngredient("B", Item::get(Item::QUARTZ, Quartz::QUARTZ_NORMAL, 1)));
		$buildRecipes[] = ((new ShapedRecipe(Item::get(Item::STONE, Stone::DIORITE, 2), ...$shapes["blockrecipe2X2"]))->setIngredient("A", Item::get(Item::COBBLESTONE, 0, 2))->setIngredient("B", Item::get(Item::QUARTZ, 0, 2)));
		$buildRecipes[] = ((new ShapedRecipe(Item::get(Item::STONE, Stone::ANDESITE, 2), ...$shapes["blockrecipe1X2"]))->setIngredient("A", Item::get(Item::COBBLESTONE, 0, 1))->setIngredient("B", Item::get(Item::STONE, Stone::DIORITE, 1)));
		$buildRecipes[] = ((new ShapedRecipe(Item::get(Item::STONE_BRICK, StoneBricks::MOSSY, 1), ...$shapes["blockrecipe1X2"]))->setIngredient("A", Item::get(Item::STONE_BRICK, StoneBricks::NORMAL, 1))->setIngredient("B", Item::get(Item::VINES, 0, 1)));

		$this->sortAndAddRecipesArray($buildRecipes);
	}

	private function createOneIngredientRecipe($recipeShape, $resultItem, $resultItemMeta, $resultItemAmount, $ingredientType, $ingredientMeta, $ingredientName, $inventoryType = "") {
		$ingredientAmount = 0;
		$height = 0;
		...
		$recipe = null;
		if($height < 3) {
			$recipe = ((new ShapedRecipe(Item::get($resultItem, $resultItemMeta, $resultItemAmount), ...$recipeShape))->setIngredient($ingredientName, Item::get($ingredientType, $ingredientMeta, $ingredientAmount)));
		} else {
			$recipe = ((new BigShapedRecipe(Item::get($resultItem, $resultItemMeta, $resultItemAmount), ...$recipeShape))->setIngredient($ingredientName, Item::get($ingredientType, $ingredientMeta, $ingredientAmount)));
		}
	...
	private function sortAndAddRecipesArray(&$recipes) {
		// Sort the recipes based on the result item name with the bubble sort algorithm.
	...
	protected function registerIngots() {
		$ingots = [Item::GOLD_BLOCK => Item::GOLD_INGOT, Item::IRON_BLOCK => Item::IRON_INGOT, Item::DIAMOND_BLOCK => Item::DIAMOND, Item::EMERALD_BLOCK => Item::EMERALD, Item::REDSTONE_BLOCK => Item::REDSTONE_DUST, Item::COAL_BLOCK => Item::COAL, Item::HAY_BALE => Item::WHEAT,];
	...
	protected function registerWeapons() {
		$types = [[Item::WOODEN_PLANK, Item::COBBLESTONE, Item::IRON_INGOT, Item::DIAMOND, Item::GOLD_INGOT], [Item::WOODEN_SWORD, Item::STONE_SWORD, Item::IRON_SWORD, Item::DIAMOND_SWORD, Item::GOLD_SWORD],];

		for($i = 1; $i < 2; ++$i) {
			foreach($types[$i] as $j => $type) {
				$this->registerRecipe((new BigShapedRecipe(Item::get($type, 0, 1), " X ", " X ", " I "))->setIngredient("X", Item::get($types[0][$j], null))->setIngredient("I", Item::get(Item::STICK)));
			}
		}

		$this->registerRecipe((new BigShapedRecipe(Item::get(Item::ARROW, 0, 1), " F ", " S ", " P "))->setIngredient("S", Item::get(Item::STICK))->setIngredient("F", Item::get(Item::FLINT))->setIngredient("P", Item::get(Item::FEATHER)));

		$this->registerRecipe((new BigShapedRecipe(Item::get(Item::BOW, 0, 1), " X~", "X ~", " X~"))->setIngredient("~", Item::get(Item::STRING))->setIngredient("X", Item::get(Item::STICK)));
	}
